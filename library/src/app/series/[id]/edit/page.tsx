...
export default async function EditSeriesPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const updateActionWithId = updateSeries.bind(null, id)

  return (
    <main className="min-h-screen bg-gray-50 p-8">
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <header className="flex justify-between items-center mb-8">
          <h1 className="text-2xl font-bold text-gray-900">Editar Saga</h1>
        ...
        <form action={updateActionWithId} className="flex flex-col gap-6">
          <div>
            ...
            <input id="name" name="name" type="text" required defaultValue={series.name} className="w-full px-3 py-2 border border-gray-300 rounded-md text-black shadow-sm focus:border-gray-900 focus:ring-1 focus:ring-gray-900" />
          </div>
